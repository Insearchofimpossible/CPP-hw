...
class counter {
private:
    inline static size_t objects_count = 0;
protected:
    counter() { ++objects_count; }
    counter(const counter&) { ++objects_count; }
    counter(counter&&) noexcept { ++objects_count; }
    ~counter() { --objects_count; }
public:
    static size_t count() { return objects_count; }
};
